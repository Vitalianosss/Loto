...
var bollWidth = rootStyles.getPropertyValue('--bollWidth');
var backgroundColor = rootStyles.getPropertyValue('--color-dark-background');

let bollWidthArray = [...bollWidth];
for (i = 0; i < 2; i++) {
    bollWidthArray.pop();
}
bollWidth = +bollWidthArray.join('') + 20 * 2;

function getSettingsCounter() {
    if (window.outerHeight > 700) {
        if (swiperCounterBochonkov.realIndex == 0) {
            counterBochonkov = +(numbers.clientWidth / bollWidth + 2).toFixed(0);
            console.log(counterBochonkov);
        } else {
            counterBochonkov = swiperCounterBochonkov.realIndex + 2;
        }
    } else {
        if (swiperCounterBochonkov.realIndex == 0) {
            counterBochonkov = +(numbers.clientWidth / bollWidth + 3).toFixed(0);
            console.log(counterBochonkov);
        } else {
            counterBochonkov = swiperCounterBochonkov.realIndex + 2;
        }
